...
class Token:
    ...
    def __repr__(self):
        return "Token(%s, %s)" % (self.word, self.pos)
...
class LabeledSentence:
    ...
    Attributes:
        ...
        labels: list[int]
    """
    def __init__(self, tokens: List[Token], labels: List[int]):
        self.tokens = tokens
        self.labels = labels
        # if chunks is None:
        #     self.bio_tags = None
    ...
    for line in f:
        ...
        if stripped != "":
            ...
            for i in range(1,len(tokens)):
                curr_tokens.append(Token(tokens[i], pos_tags[i]))
            curr_labels.append(tokens[0])
            # print('Tokens:', curr_tokens)
            # print('Labels:', curr_labels)
        if len(curr_tokens) > 0:
            sentences.append(LabeledSentence(curr_tokens, curr_labels)) #chunks_from_bio_tag_seq(curr_bio_tags)))
            curr_tokens = []
            curr_labels = []
